...
object TransportProblem extends App with SparkSessionWrapper{
  ...
  val ON_TIME = "onTimeFlights"
  val SMALL_DELAY = "smallDelayFlights"
  val LARGE_DELAY = "largeDelayFlights"
  val TOTAL = "totalFlights"
  ...
  def main(args: Array[String]) {
    ...
    val resultAirDelay = AirlineDelayDetails.as[AirlineDelay]

    val resultSet = delayedAirlines(resultAirDelay, Some("2018"))
  }
 ...
 (en caso de no recibir el parametro de todos los años)
  ...
  def delayedAirlines(ds: Dataset[AirlineDelay], year: Option[String]): Seq[AirlineStats] = {
    /*val dsr = ds.groupBy("airline")
                ...
                .show()*/
    val dsf = year match {
      case Some(y) => ds.filter(_.fl_date.contains(y))
      case None => ds
    }

    val airlines = spark.read.option("header", "true").csv("airlines.csv")

    dsf.map( d => (d.airline, d.arr_delay))
      .groupBy("_1")
      .agg(
        count(lit(1)).alias(TOTAL),
        count(when($"_2" > 45, 1)).alias(LARGE_DELAY),
        count(when($"_2" > 5 && $"_2" < 45, 1)).alias(SMALL_DELAY),
        count(when($"_2" < 5, 1)).alias(ON_TIME)
      )
